...
def orientation_quat():
    return tf_transformations.quaternion_from_euler(0, 0, 0)
...
class BlueSquarePbvs(Node):
    def __init__(self):
        ...
        self.sweep_waypoints = [
            (-0.4, -0.15, 0.60), (-0.4,  0.15, 0.60),
            ( 0.0,  0.15, 0.60), ( 0.4,  0.15, 0.60),
            ( 0.4, -0.15, 0.60), ( 0.0, -0.15, 0.60),
        ]
        self.sweep_idx = 0
        self.busy = False
        ...
        # --- FIX: Add an initialization flag to prevent race conditions ---
    ...
    def initial_start_callback(self):
        ...
        # --- FIX: Set the flag to True only AFTER setup is complete ---
    ...
    def square_cb(self, msg):
        # --- FIX: Do not process any poses until the node is initialized ---
        ...
        if self.busy:
            # If we are already servoing, just update the target's position.
            # The running pbvs_step loop will pick up this new coordinate.
    ...
    def pbvs_step(self):
        ...
        try:
            ...
            if xy_err < 0.02 and abs(z_err) < 0.01:
    ...
    def send_pose_goal_async(self, pose_stamped, done_callback):
        ...
        ori_constraint.absolute_z_axis_tolerance = np.pi
