...
class BlueSquarePbvs(Node):
    ...
    def square_cb(self, msg):
        ...
        for i, sq in enumerate(self.visited_squares):
            ...
            if dist < 0.25:  # Increased exclusion zone to 25cm
    ...
    def sweep_loop(self):
        ...
        self.get_logger().info(f"Sweeping to waypoint {self.sweep_idx}: ({x:.2f}, {y:.2f}, {z:.2f}) | Visited: {len(self.visited_squares)} squares")
            ...
            if self.last_target_update_time is not None:
                ...
                try:
                    ...
                    if distance_to_target < 0.15:  # Very close - vision likely failed due to depth issues
                        timeout_threshold = 20.0  # 20 seconds - very tolerant
                        ...
                        timeout_threshold = 12.0  # 12 seconds
                        if self.pbvs_iter > 3 and timeout_seconds > timeout_threshold:
    ...
    def send_pose_goal_async(self, pose_stamped, done_callback):
        ...
        pos_constraint.constraint_region.primitives.append(SolidPrimitive(type=SolidPrimitive.SPHERE, dimensions=[0.025]))  # Tighter tolerance
