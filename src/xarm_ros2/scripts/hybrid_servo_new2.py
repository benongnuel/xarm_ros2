...
class HybridServoPlan(Node):
    ...
    def main_loop(self):
        ...
        if self.state == 'sweep':
            ...
            # ...existing code inside main_loop, in the 'servo' state...

            ee = trans.transform.translation
            q = trans.transform.rotation

            # Offset: 10cm along Z+ of link6 in link6 frame
            tcp_offset = np.array([0, 0, 0.1])  # 10cm along Z
            rot = quaternion_matrix([q.x, q.y, q.z, q.w])
            tcp_in_base = np.dot(rot[:3, :3], tcp_offset) + np.array([ee.x, ee.y, ee.z])

            # Error from TCP (tip) to target
            dx_tip = tgt.x - tcp_in_base[0]
            dy_tip = tgt.y - tcp_in_base[1]
            dz_tip = (tgt.z + self.z_offset) - tcp_in_base[2]
            xy_error_tip = np.linalg.norm([dx_tip, dy_tip])
            dist_tip = np.linalg.norm([dx_tip, dy_tip, dz_tip])

            # Error from flange (for control)
            dx = tgt.x - ee.x
            dy = tgt.y - ee.y
            dz = (tgt.z + self.z_offset) - ee.z
            xy_error_flange = np.linalg.norm([dx, dy])
            dist_flange = np.linalg.norm([dx, dy, dz])

            # Use dx, dy, dz for control (as before)
            # Log both errors
            self.get_logger().info(
                f"[DEBUG] Target (x, y, z): ({tgt.x:.3f}, {tgt.y:.3f}, {tgt.z:.3f}), "
                f"EE (x, y, z): ({ee.x:.3f}, {ee.y:.3f}, {ee.z:.3f}), "
                f"XY error (flange): {xy_error_flange:.3f}, XY error (tip): {xy_error_tip:.3f}, "
                f"dist (flange): {dist_flange:.3f}, dist (tip): {dist_tip:.3f}"
            )

            if self.start_servo_time is not None:
                xy_error = xy_error_flange  # For logging and deadzone logic, keep as flange error
                elapsed_time = time.time() - self.start_servo_time
                self.error_log.append(xy_error)
                self.time_log.append(elapsed_time)

            dist = dist_flange  # Use flange distance for control logic
            # ---------- DEBUG PRINT --------------
            ...
            if dist < self.hold_distance:
                if self.hold_start_time is None:
                    ...
                    self.get_logger().info(
                        f"[Servo Stop] Reached hold position in {time_to_target:.2f} seconds."
                    )
                    self.start_servo_time = None  # <-- Reset here!
                    self.get_logger().info(
                        f"Within hold_distance ({self.hold_distance:.3f} m), holding for {self.hold_duration:.1f} seconds."
                    )
