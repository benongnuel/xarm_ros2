...
class BlueSquarePbvs(Node):
    def __init__(self):
        ...
        self.square_detection_time = None  # Timer variable
    ...
    def square_cb(self, msg):
        ...
        try:
            ...
            # Correct usage: do_transform_pose expects Pose and TransformStamped
        ...
        self.busy = True
        self.square_detection_time = time.time()  # Start timer here
    ...
    def pbvs_step(self):
        ...
        try:
            if self.pbvs_target_pose is None:
                self.get_logger().warn("No PBVS target pose set. Resetting.")
                self.reset_state()
                return

            target = self.pbvs_target_pose.pose.position
            target_z_eef = min(target.z - 0.01, 0.98)
            trans = self.tf_buffer.lookup_transform('link_base', 'link_eef', rclpy.time.Time())
            ee_pos = trans.transform.translation
            q = trans.transform.rotation

            # Offset: 0.1m along Z+ of link_eef (your TCP offset)
            tcp_offset = np.array([0, 0, 0.1, 1])  # Adjust the 0.1 if your offset is different!
            rot = tf_transformations.quaternion_matrix([q.x, q.y, q.z, q.w])
            tcp_in_base = np.dot(rot, tcp_offset)[:3] + np.array([ee_pos.x, ee_pos.y, ee_pos.z])

            xy_err = np.linalg.norm([target.x - tcp_in_base[0], target.y - tcp_in_base[1]])
            z_err = target_z_eef - tcp_in_base[2]

            self.get_logger().info(f"[PBVS {self.pbvs_iter+1}/{self.pbvs_max_iters}] XY_err: {xy_err*100:.1f} cm, Z_err: {z_err*100:.1f} cm")

            if not hasattr(self, "xy_within_done_threshold_since"):
                self.xy_within_done_threshold_since = None

            if xy_err < 0.02 and abs(z_err) < 0.02:
                if self.xy_within_done_threshold_since is None:
                    self.xy_within_done_threshold_since = time.time()
                elif time.time() - self.xy_within_done_threshold_since > 1.0:
            ...
            if xy_err > 0.03:
                ...
                xy_dir = [(target.x - tcp_in_base[0]) / xy_err, (target.y - tcp_in_base[1]) / xy_err]

                next_pose.pose.position.x = tcp_in_base[0] + xy_dir[0] * xy_step
                next_pose.pose.position.y = tcp_in_base[1] + xy_dir[1] * xy_step
                next_pose.pose.position.z = tcp_in_base[2]
                ...
                next_pose.pose.position.x = tcp_in_base[0]
                next_pose.pose.position.y = tcp_in_base[1]
                next_pose.pose.position.z = tcp_in_base[2] + z_step
    ...
    def reset_state(self):
        ...
        self.current_goal_handle = None
        self.last_target_update_time = None
        self.square_detection_time = None  # Reset timer
