...
class RedBallServoPBVS(Node):
    def __init__(self):
        ...
        # For stuck/singularity detection
        self.prev_error = None
        self.stuck_count = 0
        self.stuck_threshold = 10  # Number of cycles with no progress before switching
        ...
        self.servo_to_plan_thresh = 0.10  # 10cm for hybrid switch
    ...
    def main_loop(self):
        if self.state == 'sweep':
            if not self.move_in_progress:
                ...
                qx, qy, qz, qw = quaternion_from_euler(np.pi, 0, 0)
            ...
            tz = self.target_pose.pose.position.z + self.fixed_z
            dx = tx - ee_x
            dy = ty - ee_y
            dz = tz - ee_z
            error = np.linalg.norm([dx, dy, dz])

            # Stuck/singularity detection
            if self.prev_error is not None and abs(error - self.prev_error) < 1e-3:
                self.stuck_count += 1
            else:
                self.stuck_count = 0
            ...
            if self.stuck_count > self.stuck_threshold:
                self.get_logger().warn("Servo appears stuck (possible singularity or joint limit). Switching to MoveIt planning.")
                self.state = 'plan'
                self.stuck_count = 0
                return
            ...
            # Hybrid: switch to planning for final approach
            if error < self.servo_to_plan_thresh:
            ...
            if not self.move_in_progress:
                ...
                if self.target_pose:
                    ...
                    final_pose.pose = self.target_pose.pose
                    final_pose.pose.position.z += self.fixed_z + self.stop_threshold
                    qx, qy, qz, qw = quaternion_from_euler(np.pi, 0, 0)
