...
class HybridServoPlan(Node):
    def __init__(self):
        ...
        self.deadzone = 0.05              # Do not servo if closer than 5cm
    ...
    def main_loop(self):
        ...
        if self.state == 'sweep':
            ...
            holding = self.hold_start_time is not None

            # Only exit on vision loss if NOT holding
            if not holding and (not self.locked_target_pose or (now - self.last_detection_time > 2.0)):
                self.get_logger().warn("Lost target or vision timeout, returning to sweep.")
                self.state = 'sweep'
                return

            # --- PATCH: Hold when within 5cm for 10 seconds ---
            self.hold_distance = 0.25  # meters
            self.hold_margin = 0.002  # meters, how close to hold

            if not hasattr(self, 'hold_start_time'):
            ...
            if dist < self.hold_distance:
                if self.hold_start_time is None:
                    ...
                    self.get_logger().info("Within 20cm of target, starting hold timer.")
                elif time.time() - self.hold_start_time >= self.hold_duration:
                    self.get_logger().info("Hold complete. Returning to sweep.")
                    # Optionally publish/log the last pose here if you want
                    self.state = 'sweep'
                    self.hold_start_time = None
                    return
                else:
